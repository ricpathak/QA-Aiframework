...
export function getTestHealth(server: McpServer): void {
	server.registerTool(
		"get_test_failures",
		{
			...
			inputSchema: {
				project: z.string().optional().describe("Filter by Playwright project name, e.g. 'chromium'. Omit for all."),
			},
		},
		async ({ project }) => {
			...
			const filtered = project ? failures.filter((f) => (f as Record<string, string>).project === project) : failures;
	...
	server.registerTool(
		...
		{
			description:
				"Check the heal history of a test file before attempting to fix it. If healCount >= 3, regenerate from the source plan instead of healing again.",
			inputSchema: {
				testFile: z.string().describe("Relative path to the test file, e.g. 'src/tests/ui/todo.spec.ts'"),
			},
		},
		async ({ testFile }) => {
			...
			const history = record.healHistory
				.map(
					(h: { date: string; fix: string }, i: number) =>
						`  ${i + 1}. [${new Date(h.date).toLocaleDateString()}] ${h.fix}`,
				)
				.join("\n");
	...
	server.registerTool(
		...
		{
			description:
				"List all tracked test files with their heal counts. Use this to identify tests that need regeneration across the whole suite.",
		},
		async () => {
			const data = readHealthData();
		...
		"record_heal_event",
		{
			...
			inputSchema: {
				...
				fix: z
					.string()
					.describe(
						"Short description of what was fixed, e.g. 'Updated selector from getByRole contentinfo to locator .todoapp footer'",
					),
				healType: z
				...
				approvalNote: z
					...
					.optional()
					.describe(
						"Required when healType='assertion'. Include who approved and why the assertion change is correct.",
					),
				planSource: z
					.string()
					.optional()
					.describe("Path to the spec plan this test was generated from, e.g. 'specs/todo-plan.md'"),
			},
		},
		async ({
			testFile,
