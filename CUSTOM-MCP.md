...
**Agent usage (in `.github/agents/*.agent.md`):**
Declare only tool names in `tools:` (for example, `playwright-qa-context/get_framework_conventions`).
Local MCP server wiring comes from `.vscode/mcp.json`; agent frontmatter does not configure MCP servers.
