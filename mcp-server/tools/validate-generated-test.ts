...
function validateTestFile(content: string, filePath: string): ValidationResult[] {
	...
	testCallLines.forEach((lineNum) => {
		...
		if (!window.includes("tag:") && !window.includes("TAGS.")) {
			results.push({
				...
				message:
					"test() call missing tag. Add { tag: [TAGS.SMOKE] } or { tag: [TAGS.REGRESSION] }. Import TAGS from '@utils/configuration'.",
				line: lineNum,
			});
		}
	});
	...
	if (hasOnlyExistenceAssertions) {
		results.push({
			...
			message:
				"All assertions only check visibility. Add value assertions: toHaveText(), toHaveCount(), toHaveValue().",
		});
	}
...
export function validateGeneratedTest(server: McpServer): void {
	server.registerTool(
		...
		{
			description:
				"Validate a generated test file against framework conventions. Returns FAIL violations (must fix — blocked by pre-commit) and WARN suggestions. Call this after generator_write_test and fix all FAILs before reporting done.",
			inputSchema: {
				testFilePath: z
					.string()
		...
		async ({ testFilePath }) => {
			...
			// Count direct expect() calls AND page object expectation methods
			const directExpects = (content.match(/\bexpect\s*\(/g) ?? []).length;
			const pageObjectExpects = (content.match(/\.(expect[A-Z]\w+|assert\w+)\s*\(/g) ?? []).length;
			const currentAssertions = directExpects + pageObjectExpects;
